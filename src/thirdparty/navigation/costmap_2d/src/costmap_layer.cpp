...
void CostmapLayer::clearArea(int start_x, int start_y, int end_x, int end_y)
{
  ...
  for(int x=0; x<(int)getSizeInCellsX(); x++){
    ...
    for(int y=0; y<(int)getSizeInCellsY(); y++){
      ...
      if(toAKMcost(grid[index])!=AKM_COST_NO_INFORMATION){
        grid[index] = toOri(AKM_COST_NO_INFORMATION, AKM_OPTION_INIT);
      }
    }
  }
}
...
void CostmapLayer::updateWithMax(costmap_2d::Costmap2D& master_grid, int min_i, int min_j, int max_i, int max_j)
{
  ...
  for (int j = min_j; j < max_j; j++)
  {
    ...
    for (int i = min_i; i < max_i; i++)
    {
      ...
      if (grid_value.cost == AKM_COST_NO_INFORMATION){
        it++;
        continue;
      }

      auto old_grid_value = toAKMgrid(master_array[it]);
      if (old_grid_value.cost == AKM_COST_NO_INFORMATION || old_grid_value.cost < grid_value.cost) {
        setAKMcost(master_array[it], grid_value.cost);
      }
      if (grid_value.cost == AKM_COST_LETHAL_OBSTACLE && old_grid_value.option < grid_value.option) {
        setAKMoption(master_array[it], grid_value.option);
      }
      it++;
    }
  }
...
void CostmapLayer::updateWithAddition(costmap_2d::Costmap2D& master_grid, int min_i, int min_j, int max_i, int max_j)
{
  ...
  for (int j = min_j; j < max_j; j++)
  {
    ...
    for (int i = min_i; i < max_i; i++)
    {
      ...
      auto old_grid_value = toAKMgrid(master_array[it]);
      if (old_grid_value.cost == AKM_COST_NO_INFORMATION)
        master_array[it] = costmap_[it];
      else {
        int sum = old_grid_value.cost + grid_value.cost;
        if (sum > AKM_COST_INSCRIBED_INFLATED_OBSTACLE)
          setAKMcost(master_array[it], AKM_COST_INSCRIBED_INFLATED_OBSTACLE - 1);
        else
          setAKMcost(master_array[it], sum);
      }
      if (grid_value.cost == AKM_COST_LETHAL_OBSTACLE && old_grid_value.option < grid_value.option) {
        setAKMoption(master_array[it], grid_value.option);
      }
