...
namespace ORB_SLAM3 {
    ...
    pcl::PointCloud< PointCloudMapping::PointT >::Ptr PointCloudMapping::generatePointCloud(KeyFrame *kf,const cv::Mat& imRGB, const cv::Mat& imD, const cv::Mat& pose)
    {
        ...
        int CurrentKfId = 0;
        cout << " kf->mvObjects.size === " <<  kf->mvObjects.size() << endl;
        cout << "CurrentKfId === " <<  CurrentKfId << endl;
        for(size_t v = 80; v < 400 ; v+=5){
            for(size_t u = 80; u < 560 ; u+=5){
                cv::Point2i pt(u,v);
                bool isDynamic = false;
                float d = imD.ptr<float>(v)[u];
                if(d < 0.1 || d>10)
                    continue;
                if(!kf->mvObjects.empty())
                {
                    for(auto object: kf->mvObjects[CurrentKfId]){
                        if(object.class_idx == 0 && object.bbox.contains(pt))
                            isDynamic = true;
                        if(isDynamic)
                            break;
                    }
