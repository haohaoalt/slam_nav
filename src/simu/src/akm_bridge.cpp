...
void AkmBridge::init() {
  ...
  control_sub_ = nh.subscribe(CommandTopic, 1, &AkmBridge::control_callback, this);
  feedback_sub_ = nh.subscribe(FeedbackTopic, 1, &AkmBridge::feedback_callback, this, ros::TransportHints().tcpNoDelay());
  fusion_analysis_timer_ = nh.createTimer(ros::Duration(TimerDuration), &AkmBridge::timer_callback, this);
}

void AkmBridge::control_callback(const geometry_msgs::Twist::ConstPtr& msg) {
...
void AkmBridge::feedback_callback(const nav_msgs::Odometry::ConstPtr& msg) {
...
void AkmBridge::timer_callback(const ros::TimerEvent& e) {
