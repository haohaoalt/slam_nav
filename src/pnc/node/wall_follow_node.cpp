...
public:
  WallFollow() : direction_(WallFollowDirection::RIGHT),
    ...
    scan_sub_ = nh.subscribe("scan", 1, &akm::pnc::WallFollow::scan_callback, this, ros::TransportHints().tcpNoDelay());
...
private:
  ...
  void scan_callback(const sensor_msgs::LaserScan::ConstPtr& msg) {
    ...
    wf_points.header.frame_id = "ORB_SLAM3";
...
std::shared_ptr<akm::pnc::WallFollow> wf_ptr;
...
int main(int argc, char** argv) {
  ros::init(argc, argv, "akm_wall_follow");

  wf_ptr = std::make_shared<akm::pnc::WallFollow>();
