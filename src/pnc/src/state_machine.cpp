...
void StateMachine::init() {
  ...
  coverage_srv_ = nh.serviceClient<coverage_path_planner::GetPathInZone>("/akm_zone_path");
  ...
  ROS_INFO("Akm state machine initialized!");
}
...
auto StateMachine::robot_pose() -> std::optional<geometry_msgs::Pose> {
  ...
  try {
    transformStamped = tf_->lookupTransform("map", "ORB_SLAM3", ros::Time(0));
...
auto StateMachine::task_service(akm_pnc::akm_task::Request& req, akm_pnc::akm_task::Response& resp) -> bool {
  std::string root_path;
  ROS_ERROR_COND(!ros::param::get("/root_path", root_path), "Get root path failed!");
  switch (req.type) {
    case akm_pnc::akm_task::Request::EXECUTE: {
      ...
      if (req.command == akm_pnc::akm_task::Request::PAUSE) {
      ...
      if (req.command == akm_pnc::akm_task::Request::STOP) {
      ...
      if (req.command == akm_pnc::akm_task::Request::START) {
    ...
    case akm_pnc::akm_task::Request::RECORD: {
      switch (req.command) {
        case akm_pnc::akm_task::Request::START: {
        ...
        case akm_pnc::akm_task::Request::KEEP_TEACH:
        case akm_pnc::akm_task::Request::KEEP_COVER_ZZ:
        case akm_pnc::akm_task::Request::KEEP_COVER_BS:
        case akm_pnc::akm_task::Request::DISCARD: {
          if (cur_state_ != StateValue::Record) {
          ...
          cur_state_ = StateValue::Idle;
          if (req.command == akm_pnc::akm_task::Request::DISCARD) {
            resp.message = "Do not keep path, discard recorded data.";
          } else {
            if (req.command == akm_pnc::akm_task::Request::KEEP_TEACH) {
              ...
              r.request.type = req.command == akm_pnc::akm_task::Request::KEEP_COVER_ZZ ? 0 : 1;
        ...
        case akm_pnc::akm_task::Request::KEEP_TRAFFIC_ROUTE : {
    ...
    case akm_pnc::akm_task::Request::LOAD_TRAFFIC_ROUTE: {
      lines_type lines;
      auto dir = root_path + "/map/";
      auto map_path = dir + req.map + "_traffic_route.txt";
    ...
    case akm_pnc::akm_task::Request::QR_NAV: {
      using goalState = actionlib::SimpleClientGoalState;
