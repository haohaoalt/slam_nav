...
AkmRelo::AkmRelo() {
  ...
  initial_pose_sub_ = nh.subscribe("/initialpose_ori", 1, &AkmRelo::initialPoseReceived, this);
  map_sub_ = nh.subscribe("/map", 1, &AkmRelo::mapReceived, this);
  laser_sub_ = nh.subscribe("/scan", 1, &AkmRelo::laserReceived, this);
}

void AkmRelo::initialPoseReceived(const geometry_msgs::PoseWithCovarianceStampedConstPtr& msg) {
  on_going_ = true;
  auto best_pose = *msg;
...
void AkmRelo::mapReceived(const nav_msgs::OccupancyGridConstPtr& msg) {
  if (on_going_) return;
  map_ = *msg;

}

void AkmRelo::laserReceived(const sensor_msgs::LaserScanConstPtr& msg) {
  if (on_going_) return;
  laser_ = *msg;
...
auto AkmRelo::rangeRelocate(geometry_msgs::PoseWithCovarianceStamped& best_pose,
...
int main(int argc, char** argv) {
  ros::init(argc, argv, "akm_relocation");

  akm::slam::AkmRelo _akm_relo;
  ros::spin();
